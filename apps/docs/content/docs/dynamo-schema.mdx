...
| `BCAST#product-update-04` | `META`                          | Broadcast record         |
...
broadcastPK("product-update-04"); // → "BCAST#product-update-04"
```
...
```typescript
{
  PK: "BCAST#product-update-2026-04",
  SK: "META",
...
Written by BroadcastFn after resolving the subscriber list. Query via `get_broadcast` or `list_broadcasts` MCP tools.
...
| Get broadcast record          | `PK = BCAST#broadcastId, SK = META`                            |
| List all broadcasts           | Scan where `PK begins_with BCAST#` and `SK = META`             |
