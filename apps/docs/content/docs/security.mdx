---
title: Security
description: Data protection in DynamoDB, encryption options, IAM least-privilege roles, secrets management, and compliance guidance for handling subscriber data.
---
...
| **Data TTL**               | Disabled by default. Set `DATA_TTL_DAYS` to enable automatic cleanup |
...
- **Engagement events** - If `DATA_TTL_DAYS` is set, events expire automatically. For immediate deletion, query by `PK = SUB#{email}` and delete all matching items.
