...
- **`mcp`** - MCP server (`@mailshot/mcp`) for interacting with the email system from Claude Code. Provides tools for subscriber management, engagement analytics, template preview, sequence inspection (`list_sequences` with `activeExecutionCount`, `list_sequence_subscribers` for "who is in sequence X"), system health, and skill sync (`sync_skills`). Spawned over stdio, uses local AWS credentials. Setup: `claude mcp add mailshot -e AWS_PROFILE=<profile> -- npx @mailshot/mcp` (reads config from `.env`).
- **`skills`** - `@mailshot/skills` is a content-only package that ships the canonical Claude Code `SKILL.md` files (`create-sequence`, `setup-env`, `validate-sequence`, `deploy`, `teardown`, `inspect-sequence`, `import-sequence`, `send-broadcast`) plus a tiny `mailshot-sync-skills` bin and an exported `syncSkills()` function. Scaffolded user projects depend on it and run the bin in their `postinstall` hook, so any `pnpm install` (including `pnpm update @mailshot/skills`) refreshes their `.claude/skills/`. The framework repo's root `.claude/skills/` is a symlink to `packages/skills/skills/` so contributors edit the same files end users receive — there is no separate "framework dev" copy. The bin overwrites unconditionally; git is the safety net for local edits.
- **`create`** - `create-mailshot` CLI that scaffolds new user projects. Run with `npx create-mailshot my-project`. Template files are embedded in the package. Imports `syncSkills()` from `@mailshot/skills` to drop the canonical skills into the new project at scaffold time.
...
**Main table** (single-table design): Items keyed by `PK = SUB#<email>` (subscribers), `PK = EXEC#<sequenceId>` (sequence-side execution rows, inverted partner of `SUB#email/EXEC#seq` — written transactionally by `putExecution`), `PK = TAG#<tag>` (tag inverted index), `PK = BROADCAST` (broadcast records), or `PK = STATS#<sequenceId>` (engagement counters). Subscriber sort keys: `PROFILE`, `EXEC#<sequenceId>`, `SENT#<timestamp>`, `SUPPRESSION`. Sequence-side EXEC sort key: `SUB#<email>`. Tag SK: `SUB#<email>`. Broadcast SK: `<timestamp>#<broadcastId>`. Stats SK: `COUNTERS` (single item per sequence/broadcast, atomically updated via `UpdateItem ADD`). No GSIs — reverse lookups (e.g. "all subscribers in sequence X") are served by these inverted partner items. `putExecution` and `deleteExecution` always wrap the two EXEC rows in a `TransactWriteItemsCommand` so they cannot drift. Subscriber attributes (platform, country, gateway, etc.) are stored as **top-level columns** on the PROFILE item - not nested under an `attributes` map. System columns (`PK`, `SK`, `email`, `firstName`, `unsubscribed`, `suppressed`, `createdAt`, `updatedAt`) are fixed; everything else is a dynamic attribute. Use `extractAttributes(profile)` from `dynamo-client.ts` to separate custom attributes from system columns.
...
- The MCP server derives `broadcastFnName` as `${STACK_NAME}-broadcast` automatically — no separate env var needed
