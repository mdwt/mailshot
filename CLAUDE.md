...
pnpm --filter @step-func-emailer/templates render  # Render .tsx → .html in out/
```
...
- **`templates`** — React Email components in `src/emails/`. Build step renders them to static HTML with Liquid placeholders (`{{ firstName }}`, `{{ unsubscribeUrl }}`), output to `out/`, deployed to S3 via CDK BucketDeployment.
