...
```bash
cd packages/cdk && AWS_PROFILE=<profile> npx cdk destroy --force
```

The `--force` flag skips CDK's interactive confirmation prompt, which is redundant since the user already confirmed by typing the stack name.
